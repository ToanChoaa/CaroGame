...
def evaluate_board(board, piece):
    ...
    if rows == 3 and cols == 3:
        ...
        lines = []
        # 3 Hàng ngang
        for r in range(3): lines.append(board[r])
        # 3 Hàng dọc
        for c in range(3): lines.append([board[r][c] for r in range(3)])
        # 2 Đường chéo
        lines.append([board[i][i] for i in range(3)])
        lines.append([board[i][2-i] for i in range(3)])
        
        X_n = 0 # Số đường thắng tiềm năng của quân Ta (piece)
        O_n = 0 # Số đường thắng tiềm năng của Địch (opp_piece)
        
        for line in lines:
            # Đường thắng tiềm năng của Ta: Chứa quân Ta hoặc Trống, KHÔNG chứa Địch
            if opp_piece not in line:
                X_n += 1
                
            # Đường thắng tiềm năng của Địch: Chứa Địch hoặc Trống, KHÔNG chứa Ta
...
def hard_ai_move(board, piece):
    ...
    if rows == 3:
        ...
        if empty_cells > 60:
            depth = 3   # đầu game: nhiều ô trống → sâu ít
        elif empty_cells > 35:
            depth = 4   # giữa game
        else:
...
class GameFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Điểm số và thời gian cho X và O
        self.x_score = 0
        self.o_score = 0
        self.x_times = []
        self.o_times = []
        self.x_score_label = None
        self.o_score_label = None
    ...
    def create_widgets(self):
        ...
        self.canvas.pack(padx=10, pady=10)  # Giữ nguyên như cũ
        ...
        self.score_frame.pack(pady=5)  # Giữ nguyên như cũ
        # Điểm số X
        x_frame = tk.Frame(self.score_frame, bg="#f8c9c9", relief="solid", bd=2, padx=15, pady=8)
        x_frame.pack(side="left", padx=10)
        tk.Label(x_frame, text="X", font=("Helvetica", 11, "bold"), 
                bg="#f8c9c9", fg="#781c1c").pack()
        self.x_score_label = tk.Label(x_frame, text="Điểm: 0", 
                                           font=("Helvetica", 12, "bold"), 
                                           bg="#f8c9c9", fg="#d62828")
        self.x_score_label.pack()
        
        # Điểm số O
        o_frame = tk.Frame(self.score_frame, bg="#c9d4f8", relief="solid", bd=2, padx=15, pady=8)
        o_frame.pack(side="left", padx=10)
        tk.Label(o_frame, text="O", font=("Helvetica", 11, "bold"), 
                bg="#c9d4f8", fg="#1c1c78").pack()
        self.o_score_label = tk.Label(o_frame, text="Điểm: 0", 
                                       font=("Helvetica", 12, "bold"), 
                                       bg="#c9d4f8", fg="#2828d6")
    ...
    def new_game(self, mode, difficulty, board_size):
        ...
        self.turn = PLAYER_PIECE if random.choice([True, False]) else AI_PIECE
    ...
    def back_to_menu(self):
        # Thay đổi: Ngăn AI tiếp tục thực hiện nước đi khi quay về menu chính
        self.game_over = True  # <-- Dòng mới được thêm
    ...
    def update_scores(self):
        ...
        if self.x_score_label:
            self.x_score_label.config(text=f"Điểm: {self.x_score}")
        if self.o_score_label:
            self.o_score_label.config(text=f"Điểm: {self.o_score}")
    ...
    def after_move(self):
        ...
        if winning_move(self.board, self.turn):
            ...
            if self.mode == "Human vs Human":
                ...
                winner_text = "Bạn" if self.turn == PLAYER_PIECE else "AI"
            
            # Vẽ lại bàn cờ lần cuối để hiển thị nước thắng
            self.draw_board()
            self.update()
            
            # Hiển thị cửa sổ bảng điểm số
            self.show_score_board(winner_text, "Thắng")
            messagebox.showinfo("Game Over", f"{winner_text} thắng!")
            self.status_label.config(text=f"Game Over - {winner_text} thắng!", fg="green")
            return

        # Kiểm tra hòa
        if len(get_valid_locations(self.board)) == 0:
            self.game_over = True
            # So sánh điểm để quyết định người thắng khi hòa
            if self.x_score > self.o_score:
                winner_text = "X"
                result_text = "Thắng (theo điểm)"
            elif self.o_score > self.x_score:
                winner_text = "O"
                result_text = "Thắng (theo điểm)"
            else:
                winner_text = "Hòa"
                result_text = "Hòa (điểm bằng nhau)"
            
            # Vẽ lại bàn cờ lần cuối
            self.draw_board()
            self.update()
            
            # Hiển thị cửa sổ bảng điểm số
            self.show_score_board(winner_text, result_text)
            messagebox.showinfo("Game Over", f"{winner_text} {result_text}!")
            self.status_label.config(text=f"Game Over - {winner_text} {result_text}!", fg="orange")
            return
    ...
    def show_score_board(self, winner, result):
        score_window = tk.Toplevel(self)
        score_window.title("Bảng Điểm Số")

        score_window.configure(bg=BG_COLOR)
        score_window.resizable(False, False)

        # ====== TIÊU ĐỀ ======
        title = tk.Label(
            score_window,
            text=f"KẾT QUẢ: {winner.upper()} {result.upper()}!",
        ...
        # ====== KHUNG NGOÀI ======
        outer = tk.Frame(score_window, bg="#f8c9c9",
                        relief="solid", bd=3, padx=20, pady=20)
        outer.pack(fill="both", expand=True, padx=20, pady=10)

        # ====== FRAME CHỨA 2 KHUNG NGANG ======
        two_box = tk.Frame(outer, bg="#f8c9c9")
        two_box.pack(fill="both", expand=True)

        # ====== TÍNH THỜI GIAN TB ======
        x_avg_time = sum(self.x_times) / len(self.x_times) if self.x_times else 0.0
        o_avg_time = sum(self.o_times) / len(self.o_times) if self.o_times else 0.0

        # --- TEMPLATE CHO 1 KHUNG ---
        def create_player_box(parent, bg_color, border_color, name, score, avg, moves):
            frame = tk.Frame(
                parent,
                bg=bg_color,
                relief="solid",
            ...
            tk.Label(frame, text=f"Người chơi: {name}",
                    font=("Helvetica", 18, "bold"),
                    bg=bg_color, fg=border_color).pack(pady=8)

            tk.Label(frame, text=f"Tổng điểm: {score}",
                    font=("Helvetica", 16),
                    bg=bg_color, fg=border_color).pack(pady=5)

            tk.Label(frame, text=f"Thời gian trung bình (giây): {avg:.4f}",
                    font=("Helvetica", 16),
                    bg=bg_color, fg=border_color).pack(pady=5)

            tk.Label(frame, text=f"Số nước đi: {moves}",
                    font=("Helvetica", 16),
                    bg=bg_color, fg=border_color).pack(pady=5)
        ...
        create_player_box(
            ...
            score=self.x_score,
            avg=x_avg_time,
            moves=len(self.x_times)
        )

        # ====== KHUNG O (giống y hệt X) ======
        create_player_box(
            two_box,
            bg_color="#c9d4f8",
            border_color="#1c1c78",
            ...
            score=self.o_score,
            avg=o_avg_time,
            moves=len(self.o_times)
        )

        # ====== NÚT ======
        btn_frame = tk.Frame(score_window, bg=BG_COLOR)
...
# -----------------------------
    ...
    def human_move(self, row, col):
        ...
        # Track thời gian người chơi đánh
